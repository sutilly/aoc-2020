...
data.forEach((line) => {
        let containedBags = line.split("bags contain");
        if (containedBags[1].includes("shiny gold")) {
            firstLoop.push(containedBags[0].trim());
        }
});

data.forEach(line => {
    firstLoop.forEach(color => {
        let containedBags = line.split("bags contain");
        if (containedBags[1].includes(color)) {
            secondLoop.push(containedBags[0].trim());
        }
    })
});

data.forEach(line => {
    secondLoop.forEach(color => {
        let containedBags = line.split("bags contain");
        if (containedBags[1].includes(color)) {
            thirdLoop.push(containedBags[0].trim());
        }
    })
});

data.forEach(line => {
    thirdLoop.forEach(color => {
        let containedBags = line.split("bags contain");
        if (containedBags[1].includes(color)) {
            fourthLoop.push(containedBags[0].trim());
        }
    })
});

data.forEach(line => {
    fourthLoop.forEach(color => {
        let containedBags = line.split("bags contain");
        if (containedBags[1].includes(color)) {
            fifthLoop.push(containedBags[0].trim());
        }
    })
});

data.forEach(line => {
    fifthLoop.forEach(color => {
        let containedBags = line.split("bags contain");
        if (containedBags[1].includes(color)) {
            sixthLoop.push(containedBags[0].trim());
        }
    })
});

data.forEach(line => {
    sixthLoop.forEach(color => {
        let containedBags = line.split("bags contain");
        if (containedBags[1].includes(color)) {
            seventhLoop.push(containedBags[0].trim());
        }
    })
});

data.forEach(line => {
    seventhLoop.forEach(color => {
        let containedBags = line.split("bags contain");
        if (containedBags[1].includes(color)) {
            eighthLoop.push(containedBags[0].trim());
        }
    })
});
let all = firstLoop.concat(secondLoop).concat(thirdLoop).concat(fourthLoop).concat(fifthLoop).concat(sixthLoop).concat(seventhLoop).concat(eighthLoop)
let set = [...new Set(all)];
