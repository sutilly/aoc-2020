...
data.forEach(input => {
    ...
    let groupSize = inputSanitized.split(',').length;

    let equalAnswers = 0;
    let checkedLetters = [];

    for (let j = 0; j < inputSanitized.length; j++) {
        let letter = inputSanitized.charAt(j);
        if (!checkedLetters.includes(letter)) {
            let letterOccurence = [...inputSanitized].filter(x => x === letter).length;
            if (letterOccurence === groupSize) {
                equalAnswers++;
            }
            checkedLetters.push(letter);
        }
    }
    answers+=equalAnswers;
});
