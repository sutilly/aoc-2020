...
hcl:#ceb3a1 b hcl:#006f93 iyr:2015
...
// puzzle 2
let validDataObjects2 = [];

validDataObjects.forEach(object => {
    if (checkbyr(object) && checkhgt(object) && checkiyr(object) && checkeyr(object) && checkhcl(object) && checkecl(object) && checkpid(object)) {
        validDataObjects2.push(object)
    }
});

function checkbyr(obj) {
    let byr = obj.byr * 1;
    return !(byr < 1920 || byr > 2002);
}

function checkiyr(obj) {
    let iyr = obj.iyr * 1;
    return !(iyr < 2010 || iyr > 2020);

}

function checkeyr(obj) {
    let eyr = obj.eyr * 1;
    return !(eyr < 2020 || eyr > 2030);

}

function checkhgt(obj) {
    let hgt = obj.hgt;
    if (hgt.match(/(\d)cm/)) {
        hgt = hgt.split('c')[0];
...
function checkhcl(obj) {
    let hcl = obj.hcl;
    return hcl.match(/^#[a-f0-9]{6}$/);
}

function checkecl(obj) {
    const validClrs = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"];
    return validClrs.includes(obj.ecl);
}
...
console.log(validDataObjects2.length);
