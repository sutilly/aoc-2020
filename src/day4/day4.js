...
let validDataObjects = [];
const requiredFields = ["byr", "eyr", "iyr", "hgt", "hcl", "ecl", "pid"];

// puzzle 1

let data = input.split(`\n\n`);
data.forEach(input => {
    let inputSanitized = input.split(/\r\n|\n|\s|\r/gm).join(',');
    inputSanitized = inputSanitized.split(",").join("\",\"");
    inputSanitized = inputSanitized.split(":").join("\":\"");
    inputSanitized = `{\"${inputSanitized}\"}`;
    let obj = JSON.parse(inputSanitized)

    const hasAllFields = requiredFields.every(field => obj.hasOwnProperty(field));
